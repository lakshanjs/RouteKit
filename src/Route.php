...
class Route
{
    ...
    public $req;
     ...
     // Add a property to store permissions
     public $permissions = [];
    ...
    public static function instance(Request $req)
    {
     ...
     /**
      * Store the permission requirements for the route.
      *
  ...
  /**
     * Get permissions for the current requested URI.
     *
     * @param string $uri
     * @return array|null
     */
    public function getPermissionsForRoute($uri)
    {   
        // Remove duplicate slashes and match the current URI pattern
        $uri = $this->removeDuplSlash($uri);;
        // Find permissions based on the registered route
        if (isset($this->permissions[$uri])) {
            return $this->permissions[$uri];
        }

        return null; // No permissions found for the requested URI
    }
    ...
    public function group($group, callable $callback, array $options = [])
    {
       ...
       // $callback = Closure::bind($callback, $this, get_class());
        $callback = \Closure::bind(\Closure::fromCallable($callback), $this, get_class());
...
//        }
    ...
    public function end()
    {
        ...
        if ($this->matched && count($this->routeCallback)) {
            ...
            // Store permissions for the current matched route into the request
            $currentUri = $this->req->path;  // The current request URI
            $permissions = $this->getPermissionsForRoute($currentUri);  // Get permissions for the current route

            // Insert permissions into the request object
            $this->req->permissions = $permissions;  // Store the permissions in the request object
     ...
     * @throws \Exception
     */
    public function callback($callback, array $args = [])
    {
        if (isset($callback)) {
